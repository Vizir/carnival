    ...
    def data(render_type = RENDER_TABLE)
      ...
      records.each do |record|
        ...
        if render_type == RENDER_CSV
          @presenter.fields_for_action(:csv).each do |key, field|
            data_item[i.to_s] = ac.render_to_string :formats => [:html] , :partial => '/admin/shared/list_cel', :locals => {:presenter => @presenter,:field=> key, :record=> record, :only_render_fields => true}
            i = i + 1
          end
          ...
          @presenter.fields_for_action(:index).each do |key, field|
            data_item[i.to_s] = ac.render_to_string :formats => [:html] , :partial => '/admin/shared/list_cel', :locals => {:presenter => @presenter,:field=> key, :record=> record, :only_render_fields => false}
            i = i + 1
          end
          data_item[i.to_s] = ac.render_to_string :formats => [:html], :partial => '/admin/shared/item_buttons', :locals => {:record=>record, :presenter => @presenter}
          i = i + 1
        end
        data << data_item
      end
